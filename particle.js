...
function moveKirby() {
  if (moveLeft) {
    this.x -= 5; // Decrease Kirby's x position to move it left
  }
  if (moveRight) {
    this.x += 5; // Increase Kirby's x position to move it right
  }
}
...
function checkCollision(particle) {
  const kirbyLeft = this.x + 70 * s;
  const kirbyRight = this.x + 330 * s;
